...
def build_rename(i3, app_icons, delim, length, uniq):
    ...
    def rename(i3, e):
        ...
        visible = [getattr(workdict, "name") for workdict in workdicts if getattr(workdict, "visible")]
        visworkspaces = []
        focus = ([getattr(workdict, "name") for workdict in workdicts if getattr(workdict, "focused")] or [None])[0]
        focusname = None
